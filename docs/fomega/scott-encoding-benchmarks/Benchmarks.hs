...
filterS p (SList l) =
    l nil (\x xs -> if p x then cons x (filterS p xs) else (filterS p xs))

-- does cl@ have performance implications?
quickSortS :: (Ord a) => SList a -> SList a
quickSortS cl@(SList l) = l
    nil
    (\h t -> appendS
...
head_tail_builtin = bgroup
    "builtin"
    (map
        (\m -> bench ("m = " ++ (show m)) (whnf headTail ([1 .. m] :: [Int])))
        [2, 10, 10 ^ 2, 10 ^ 3, 10 ^ 4, 10 ^ 5]
    )
...
sum_ints_builtin = bgroup
    "builtin"
    (map
        (\m -> bench ("m = " ++ (show m)) (whnf directSum ([1 .. m] :: [Int])))
        [1, 10, 10 ^ 2, 10 ^ 3, 10 ^ 4, 10 ^ 5, 10 ^ 6, 10 ^ 7]
    )

quicksort_ints :: [(Int, [Int])] -> Benchmark
quicksort_ints =
    \testData -> bgroup "quicksort" [qs_scott testData, qs_builtin testData]

qs_scott :: [(Int, [Int])] -> Benchmark
qs_scott = \testData -> bgroup
    "scott"
    (map
        (\(m, xs) -> bench ("m = " ++ (show m)) (whnf quickSortS (fromList xs)))
        testData
    )

qs_builtin :: [(Int, [Int])] -> Benchmark
qs_builtin = \testData -> bgroup
    "builtin"
    (map (\(m, xs) -> bench ("m = " ++ (show m)) (whnf quickSort xs)) testData)
main :: IO ()
main = do
    let g :: Gen [Int]
    ...
    let qs_data = map
            (\x -> (x, take x nums))
            [1, 10, 10 ^ 2, 10 ^ 3, 10 ^ 4, 10 ^ 5, 10 ^ 6, 10 ^ 7]
    defaultMainWith defaultConfig [head_tail, sum_ints, quicksort_ints qs_data]

...
references sig d = fix (references' sig) (S.singleton d)
  where
    ...
    references' sig rs =
        S.foldr S.union rs (S.map (\d -> directReferences sig d) rs)
...
mutuallyRecursive sig d1 d2 =
    S.member d1 (references sig d2) && S.member d2 (references sig d1)

-- splitting a signature up into its component mutually recursive bits.
partitionMRec :: AlgSignature -> [[Decl AlgType]]
partitionMRec sig = if S.null sig
    then []
...
solveTwo ((t1, bigT1), (t2, bigT2)) = (d1, d2)
  where
    d1 :: Decl AlgType
    d1 =
        Decl t1 (Mu t1 (bigT1 [(Var t1), (Mu t2 (bigT2 [(Var t1), (Var t2)]))]))
    d2 :: Decl AlgType
    d2 =
        Decl t2 (Mu t2 (bigT2 [(Mu t1 (bigT1 [(Var t1), (Var t2)])), (Var t2)]))
  ...
  where
    ...
    solveOne i (ti, bigTi)
        = let
              subs :: [AlgType]
              subs = map (\(Decl _ x) -> x)
                         (solve (map (fixArg i (Var ti)) (exclude ti)))
...
fixArg n x (t, bigT) = (t, bigT . (\tys -> (take n tys) ++ (x : (drop n tys))))
...
demutualize sig =
    algSignature (concatMap (solve . fpEquations) (partitionMRec sig))

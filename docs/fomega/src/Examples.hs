...
testRandom rng t d path =
  join (writeResults path <$> mapM (\i -> randomTest d i t) rng)

-- 'testFixed f rng path' computes the (before, after) data for (f i), each i in rng.
testFixed :: (Int -> Density) -> [Int] -> FilePath -> IO ()
testFixed dgen rng path =
  writeDat path (map (\i -> beforeAfter (instantiate (dgen i))) rng)
...
binarytree = algDecl
  [declExp| binarytree = all a . 1 + (binaryTree a) * a * (binaryTree a)|]
...
multi :: AlgSignature
multi =
  algSignature [typeF, typeG, typeH, tree, forest, binarytree, list, rosetree]
...
instantiate rows = algSignature $ map
  (\(i, t) -> Decl ("t" ++ (show i)) t)
 ...
 where
  count x n = if x == n
    then []
      ...
      ((replicate x 0) ++ (1 : (replicate (n - (succ x)) 0)))
        : (count (succ x) n)
...
thinCycle n = map shift (diagonal n) where shift xs = (last xs) : (init xs)
...
randomBit p = frequency [(p, elements [1]), ((100 - p), elements [0])]

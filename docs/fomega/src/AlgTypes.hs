  ...
  where
    ...
    subExcept bound t x t' = if x `elem` bound
        then t'
            ...
            (Prod t1 t2) ->
                Prod (subExcept bound t x t1) (subExcept bound t x t2)
            (Sum t1 t2) ->
                Sum (subExcept bound t x t1) (subExcept bound t x t2)
            (App t1 t2) ->
                App (subExcept bound t x t1) (subExcept bound t x t2)
            (Abs y t') -> Abs y (subExcept (y : bound) t x t')
            (All y t') -> All y (subExcept (y : bound) t x t')
            (Mu  y t') -> Mu y (subExcept (y : bound) t x t')
            (Fun t1 t2) ->
                Fun (subExcept bound t x t1) (subExcept bound t x t2)

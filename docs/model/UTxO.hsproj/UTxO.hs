...
tx ledger txOutRef = case [ t | t <- ledger, hashTx t == idTOR txOutRef ] of
    []      -> Nothing
    (t : _) -> Just t
...
unspentOutputs = foldr
    (\t unspent -> (unspent `Map.difference` lift (spentOutputsTx t))
        `Map.union` unspentOutputsTx t
    )
    Map.empty
    where lift = Map.fromSet (const ())
...
validTx t ledger = inputsAreValid && valueIsPreserved && validValuesTx t
  where
    ...
    valueIsPreserved =
        forgeTX t
            +  sum (map (fromJust . value ledger) (map refTI $ inputsTX t))
            == feeTX t
            +  sum (map valueTO (outputsTX t))
                           -- NB: the 'fromMaybe' is safe as 'inputsAreUnspent' holds if we get here

    txIn `validatesIn` txOuts = case refTI txIn `Map.lookup` txOuts of
...
balanceTx addr t ledger
    | not (t `validTx` ledger) = error "transaction not valid in ledger"
    | otherwise                = received - spent
  where
    received =
        sum [ valueTO txOut | txOut <- outputsTX t, addressTO txOut == addr ]
    spent = sum
        [ valueTO txOut

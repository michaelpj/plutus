...
updateTypedBuiltinGenSize
    :: MonadQuote m => TypedBuiltinGenT m -> TypedBuiltinGenT m
updateTypedBuiltinGenSize =
    updateTypedBuiltinGenSized TypedBuiltinSizedSize (\_ -> return ())

-- | Update a typed built-ins generator by overwriting the @boolean@s generator.
updateTypedBuiltinGenBool
    :: MonadQuote m => GenT m Bool -> TypedBuiltinGenT m -> TypedBuiltinGenT m
...
genTypedBuiltinDef =
    updateTypedBuiltinGenInt
        ...
        $ updateTypedBuiltinGenBS (genLowerBytes . Range.linear 0)
        $ updateTypedBuiltinGenSize
...
genTypedBuiltinSmall =
    updateTypedBuiltinGenInt
            (\low high -> Gen.integral $ Range.constantFrom
                0
                (iasqrt low `div` 2)
                (isqrt high `div` 2)
            )
...
genTypedBuiltinSum =
    updateTypedBuiltinGenInt
            (\low high ->
                Gen.integral $ Range.linear (low `div` 2) (high `div` 2)
            )
        $ genTypedBuiltinDef
...
genTypedBuiltinDivide =
    updateTypedBuiltinGenInt
            (\low high ->
                Gen.filter (/= 0) . Gen.integral $ Range.linear low high
            )
        $ genTypedBuiltinDef
...
genTypedBuiltinMultiplyFailure =
    updateTypedBuiltinGenInt
            (\_ high ->
                Gen.integral $ Range.linear (isqrt high + 1) (isqrt high)
            )
        $ genTypedBuiltinDef

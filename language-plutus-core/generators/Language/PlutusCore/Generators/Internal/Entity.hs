...
runPlcT
    :: Monad m => GenT m Size -> TypedBuiltinGenT m -> PlcGenT m a -> GenT m a
...
iterAppValueToTermOf (IterAppValue term _ (TypedBuiltinValue _ x)) =
    TermOf term x
...
revealUnique (Name ann name uniq) = Name
    ann
    (name <> BSL.fromStrict (encodeUtf8 . prettyText $ unUnique uniq))
    uniq
...
genSizeFrom (TypedBuiltinSized sizeEntry _) =
    return $ flattenSizeEntry sizeEntry
genSizeFrom _ = genSizeDef

-- | Generate a 'BuiltinSized'.
genBuiltinSized :: Monad m => GenT m BuiltinSized
genBuiltinSized =
    Gen.element [BuiltinSizedInt, BuiltinSizedBS, BuiltinSizedSize]
...
withTypedBuiltinGen
    :: Monad m
    => GenT m Size
    -> (forall a . TypedBuiltin size a -> GenT m c)
    -> GenT m c
withTypedBuiltinGen genSize k =
    genBuiltin genSize >>= \b -> withTypedBuiltin b k
...
withCheckedTermGen
    ...
    -> (  forall a
        . TypedBuiltin Size a
       -> Maybe (TermOf (Value TyName Name ()))
       -> GenT Quote c
       )
    -> GenT Quote c
withCheckedTermGen genTb k = withTypedBuiltinGen genSizeDef $ \tb -> do
    termWithMetaValue <- genTb tb
    termWithValue     <-
        liftQuote
        .   unsafeTypeEvalCheck
        $   TypedBuiltinValue tb
        <$> termWithMetaValue
    k tb termWithValue
...
genIterAppValue (Denotation object toTerm meta scheme) = result
  where
    ...
    go
        ...
        -> PlcGenT
               m
               (Maybe (IterAppValue head (Term TyName Name ()) r))
  ...
  where
    ...
    go context depth tb
      ...
      where
        ...
        builtinGens = BuiltinGensT
            (genSizeFrom tb)
            (flip Gen.subterm id . go context (depth - 1))
        ...
        lookupInContext tb' =
            foldMap getCompose . DMap.lookup tb' $ unDenotationContext context
        ...
        lambdaApply = withTypedBuiltinGen genSize $ \argTb -> do
            ...
            TermOf body y <- go (insertVariable name argTb x context)
                                (depth - 1)
                                tb
            -- Assemble the term.
...
withAnyTermLoose
    :: (forall a . TermOf (TypedBuiltinValue Size a) -> GenT Quote c)
    -> GenT Quote c
withAnyTermLoose k = withTypedBuiltinGen genSizeDef
    $ \tb -> genTermLoose tb >>= k . fmap (TypedBuiltinValue tb)

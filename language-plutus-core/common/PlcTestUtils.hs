...
ppCatch value =
    either (PP.pretty . show) prettyPlcClassicDebug <$> runExceptT value

goldenPlc :: GetProgram a => String -> a -> TestNested
goldenPlc name value =
    nestedGoldenVsDocM name
        $   prettyPlcClassicDebug
        <$> (rethrow $ getProgram value)

goldenPlcCatch :: GetProgram a => String -> a -> TestNested
goldenPlcCatch name value =
    nestedGoldenVsDocM name $ ppCatch $ getProgram value

goldenEval :: GetProgram a => String -> [a] -> TestNested
goldenEval name values =
    nestedGoldenVsDocM name
        $   prettyPlcClassicDebug
        <$> (rethrow $ runPlc values)

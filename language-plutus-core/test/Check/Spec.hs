...
checkTermUniques
    :: (Ord a, MonadError (UniqueError a) m) => Term TyName Name a -> m ()
checkTermUniques = Uniques.checkTerm
    (\case
        FreeVariable{} -> False
        _              -> True
    )
...
incoherentUse =
    let u       = Unique 0
        checked = runExcept $ runQuoteT $ do
            let n  = Name Ignore "yo" u
            let ty = TyName n
            let term = LamAbs (Tag 1) n (TyVar (Tag 2) ty)
...
propRenameCheck = property $ do
  ...
  where
    ...
    checkUniques = Uniques.checkProgram
        (\case
            FreeVariable{}    -> False
            IncoherentUsage{} -> False
            _                 -> True
        )

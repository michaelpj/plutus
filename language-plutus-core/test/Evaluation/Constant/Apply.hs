...
prop_applyBuiltinName
    ...
    -> Property
prop_applyBuiltinName getFinal tbn op allTbs =
    property . hoist (pure . runQuote) $ do
        let getIterAppValue =
                runPlcT genSizeDef allTbs
                    . genIterAppValue
                    $ denoteTypedBuiltinName tbn op
...
prop_applyBuiltinNameSuccess
    ...
    -> Property
prop_applyBuiltinNameSuccess = prop_applyBuiltinName getFinal
  where
    getFinal tb =
        lift . fmap ConstAppSuccess . unsafeMakeBuiltin . TypedBuiltinValue tb
...
prop_applyBuiltinNameSuccessFailure tbn x = prop_applyBuiltinName
    getFinal
    tbn
    x
    genTypedBuiltinDef
    where getFinal tb = lift . makeConstAppResult . TypedBuiltinValue tb
...
prop_applyBuiltinNameFailure
    ...
    -> TypedBuiltinGenT Quote
    -> Property
prop_applyBuiltinNameFailure = prop_applyBuiltinName getFinal
    where getFinal tb = lift . makeConstAppResult . TypedBuiltinValue tb

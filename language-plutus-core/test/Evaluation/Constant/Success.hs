module Evaluation.Constant.Success (test_constantSuccess) where
...
test_typedTakeByteStringSuccess =
    testProperty "typedTakeByteString"
        $ prop_applyBuiltinNameSuccess typedTakeByteString
...
test_typedDropByteStringSuccess =
    testProperty "typedDropByteString"
        $ prop_applyBuiltinNameSuccess typedDropByteString
...
test_applyBuiltinNameSuccess = testGroup
    "applyBuiltinNameSuccess"
    [ test_typedAddIntegerSuccess
    , test_typedSubtractIntegerSuccess
    , test_typedMultiplyIntegerSuccess
    , test_typedDivideIntegerSuccess
...
test_evalInBounds =
    testProperty "evalInBounds" . property . hoist (pure . runQuote) $ do
        mayTermWithValue <-
            ...
            $ withCheckedTermGen genTypedBuiltinDef
            $ const return
        assert $ isJust mayTermWithValue

test_constantSuccess :: TestTree
test_constantSuccess = testGroup
    "constantSuccess"
    [test_applyBuiltinNameSuccess, test_evalInBounds]

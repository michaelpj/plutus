...
module Evaluation.Constant.Failure (test_constantFailure) where
...
test_typedAddIntegerFailure =
    testProperty "typedAddInteger" $ prop_applyBuiltinNameFailure
        typedAddInteger
        (+)
...
test_typedMultiplyIntegerFailure =
    testProperty "typedMultiplyInteger" $ prop_applyBuiltinNameFailure
...
test_typedConcatenateFailure =
    testProperty "typedConcatenate" $ prop_applyBuiltinNameFailure
        typedConcatenate
        (<>)
        genTypedBuiltinConcatenateFailure

test_applyBuiltinNameFailure :: TestTree
test_applyBuiltinNameFailure = testGroup
    "applyBuiltinNameFailure"
    [ test_typedAddIntegerFailure
    , test_typedMultiplyIntegerFailure
    , test_typedConcatenateFailure
    ]
...
test_evalOutOfBounds =
    testProperty "evalOutOfBounds"
        . property
        . hoist (pure . runQuote)
        $ do
              mayTermWithValue <-
                  forAllPrettyPlcMaybeT
                  $ withCheckedTermGen genTypedBuiltinOutOfBounds
                  $ \tb mayTermWithValue ->
...
test_constantFailure = testGroup
    "constantFailure"
    [test_applyBuiltinNameFailure, test_evalOutOfBounds]

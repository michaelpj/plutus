...
module Evaluation.CkMachine (test_evaluateCk) where
...
evenAndOdd = do
    ...
    pure
        $ Apply
              ()
              (foldl' (\acc t -> TyInst () acc t) fix2 [nat1, b1, nat2, b2])
        $ TyAbs () q (Type ())
...
evenAndOddList = do
    ...
    let
        eoFunc recc = do
            ...
            pure $ LamAbs () l li1 $ Apply
                ()
                (Apply ()
                       (TyInst () (Unwrap () (Var () l)) li2)
                       (TyInst () nil nat)
                )
                recc
    ...
    oddF <- do
        ...
        t                    <- freshName () "tail"
        li1                  <- listInt
        eoFunc $ LamAbs () h nat1 $ LamAbs () t li1 $ Apply ()
                                                            (Var () evenn)
                                                            (Var () t)
...
goldenVsPretty
    :: PrettyPlc a => String -> ExceptT BSL.ByteString IO a -> TestTree
goldenVsPretty name value =
    goldenVsString name ("test/Evaluation/" ++ name ++ ".plc.golden")
        $   either
                id
                (BSL.fromStrict . encodeUtf8 . docText . prettyPlcClassicDebug)
        <$> runExceptT value

test_evaluateCk :: TestTree
test_evaluateCk = testGroup
    "evaluateCk"
    ...
    , goldenVsPretty
        "even2"
        (pure $ evaluateCk
            (runQuote $ Apply () <$> getEven <*> getBuiltinIntegerToNat 2)
        )
    , goldenVsPretty
        "even3"
        (pure $ evaluateCk
            (runQuote $ Apply () <$> getEven <*> getBuiltinIntegerToNat 3)
        )
    , goldenVsPretty
        "evenList"
        (pure $ evaluateCk

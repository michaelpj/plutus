...
main = do
    ...
    defaultMain
        (allTests plcFiles rwFiles typeFiles typeNormalizeFiles typeErrorFiles)
...
compareTerm (Wrap _ n ty t) (Wrap _ n' ty' t') =
    compareTyName n n' && compareType ty ty' && compareTerm t t'
...
compareType (TyFix _ n t) (TyFix _ n' t') =
    compareTyName n n' && compareType t t'
...
propCBOR = property $ do
    ...
    let trip     = deserialiseOrFail . serialise
...
asIO f =
    fmap (either errorgen (BSL.fromStrict . encodeUtf8) . f) . BSL.readFile
...
unitSizeOfInteger = foldMap
    (\(i, s) -> sizeOfInteger i @?= s)
    [ (0   , 1)
    , (-1  , 1)
    , (1   , 1)
    , (-127, 1)
...
testsSizeOfInteger = testGroup
    "sizeOfInteger"
    [testCase "unit" unitSizeOfInteger, testProperty "prop" propSizeOfInteger]

testsNormalizeType :: [FilePath] -> TestTree
testsNormalizeType = testGroup "golden type synthesis + normalization tests"
    . fmap (asGolden (printNormalizeType True))
...
testsRewrite =
    testGroup "golden rewrite tests"
        . fmap
              (asGolden
                  ( format
                  $ debugPrettyConfigPlcClassic defPrettyConfigPlcOptions
                  )
              )
...
testLam =
    fmap prettyPlcDefString
        . runQuote
        . runExceptT
        $ runTypeCheckM (TypeConfig True mempty Nothing)
        $ do
...
testRebindShadowedVariable =
    let
        ...
        type0    = TyForall ()
                            xName
                            typeKind
                            (TyFun () (TyForall () yName typeKind varY) varX)
        -- (all x (type) (fun (all x (type) x) x))
        type1 = TyForall ()
                         xName
                         typeKind
                         (TyFun () (TyForall () xName typeKind varX) varX)
    in
        type0 == type1
...
tests = testCase "example programs" $ fold
    ...
    , fmt "{- program " @?= Left
        (ParseErrorE (LexErr "Error in nested comment at line 1, column 12"))

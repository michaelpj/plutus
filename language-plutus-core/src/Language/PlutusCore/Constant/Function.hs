...
mapSizeEntryTypedBuiltin
    :: (SizeEntry size -> SizeEntry size')
    -> TypedBuiltin size a
    -> TypedBuiltin size' a
mapSizeEntryTypedBuiltin f (TypedBuiltinSized se tbs) =
    TypedBuiltinSized (f se) tbs
...
withTypedBuiltinSized
    :: BuiltinSized -> (forall a . TypedBuiltinSized a -> c) -> c
...
withTypedBuiltin
    :: BuiltinType size -> (forall a . TypedBuiltin size a -> c) -> c
withTypedBuiltin (BuiltinSized se b) k =
    withTypedBuiltinSized b $ k . TypedBuiltinSized se
...
typeSchemeToType = go 0
  where
    go :: Int -> TypeScheme (Type TyName ()) a r -> Quote (Type TyName ())
    go _ (TypeSchemeBuiltin tb     ) = typedBuiltinToType tb
    go i (TypeSchemeArrow schA schB) = TyFun () <$> go i schA <*> go i schB
    go i (TypeSchemeAllSize schK   ) = do
        s <-
            mapTyNameString (<> BSL.fromStrict (Text.encodeUtf8 $ prettyText i))
                <$> freshTyName () "s"
...
dynamicBuiltinNameMeaningToType
    :: DynamicBuiltinNameMeaning -> Quote (Type TyName ())
dynamicBuiltinNameMeaningToType (DynamicBuiltinNameMeaning sch _) =
    typeSchemeToType sch
...
insertDynamicBuiltinNameDefinition
    ...
    -> DynamicBuiltinNameMeanings
insertDynamicBuiltinNameDefinition (DynamicBuiltinNameDefinition name mean) (DynamicBuiltinNameMeanings nameMeans)
    = DynamicBuiltinNameMeanings $ Map.insert name mean nameMeans

-- | Apply a continuation to the typed version of a 'BuiltinName'.
withTypedBuiltinName
    :: BuiltinName -> (forall a r . TypedBuiltinName a r -> c) -> c

...
dynamicCallAssign tb name f =
    DynamicBuiltinNameDefinition name $ DynamicBuiltinNameMeaning sch sem
  where
...
dynamicCallTypeScheme
    :: (forall size . TypedBuiltin size a)
    -> (forall size . TypeScheme size (a -> ()) ())
dynamicCallTypeScheme tb = TypeSchemeBuiltin tb
    `TypeSchemeArrow` TypeSchemeBuiltin (TypedBuiltinDyn @())

...
mkTermLet
    ...
    -> Term tyname name a
mkTermLet x (Def (VarDecl _ name ty) bind) body =
    Apply x (LamAbs x name ty body) bind
...
mkTypeLet
    ...
    -> Term tyname name a -- ^ The body of the let, possibly referencing the name.
    -> Term tyname name a
mkTypeLet x (Def (TyVarDecl _ name k) bind) body =
    TyInst x (TyAbs x name k body) bind
...
mkIterLamAbs
    :: a -> [VarDecl tyname name a] -> Term tyname name a -> Term tyname name a
mkIterLamAbs x args body =
    foldr (\(VarDecl _ n ty) acc -> LamAbs x n ty acc) body args

-- | Type abstract a list of names.
mkIterTyAbs
    :: a -> [TyVarDecl tyname a] -> Term tyname name a -> Term tyname name a
mkIterTyAbs x args body =
    foldr (\(TyVarDecl _ n k) acc -> TyAbs x n k acc) body args
...
mkIterTyForall x args body =
    foldr (\(TyVarDecl _ n k) acc -> TyForall x n k acc) body args

-- | Lambda abstract a list of names.
mkIterTyLam :: a -> [TyVarDecl tyname a] -> Type tyname a -> Type tyname a
mkIterTyLam x args body =
    foldr (\(TyVarDecl _ n k) acc -> TyLam x n k acc) body args

...
throwCkMachineException
    :: MachineError NoDynamicBuiltinNamesMachineError
    -> Term TyName Name ()
    -> a
...
instantiateEvaluate
    :: Context -> Type TyName () -> Term TyName Name () -> EvaluationResult
...
instantiateEvaluate stack ty fun
    ...
    | otherwise = throwCkMachineException
...
applyEvaluate
    ...
    -> Value TyName Name ()
    -> EvaluationResult
applyEvaluate stack (LamAbs _ name _ body) arg =
    stack |> substituteDb name arg body
applyEvaluate stack fun arg
    = let term = Apply () fun arg
      in
          case termAsPrimIterApp term of
              Nothing -> throwCkMachineException
              ...
              Just (IterApp (StaticStagedBuiltinName name) spine) ->
                  case applyEvaluateCkBuiltinName name spine of
                      ...
                      ConstAppError err     -> throwCkMachineException
                          (ConstAppMachineError err)
                          term

applyEvaluateCkBuiltinName
    :: BuiltinName -> [Value TyName Name ()] -> ConstAppResult
applyEvaluateCkBuiltinName name =
    runIdentity
        . runEvaluate (const $ Identity . evaluateCk)
        . runQuoteT

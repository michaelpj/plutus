...
markNonFreshType
    :: (HasUnique (tyname a) TypeUnique, MonadQuote m) => Type tyname a -> m ()
...
collectTypeUniques
    :: (HasUnique (tyname a) TypeUnique) => Type tyname a -> Set.Set Unique
collectTypeUniques = cata f
  where
    f = \case
...
collectTermUniques
    ...
    => Term tyname name a
    -> Set.Set Unique
collectTermUniques = cata f
  where
    f = \case
        VarF _ n -> Set.singleton (n ^. unique . coerced)
        LamAbsF _ n ty t ->
            Set.insert (n ^. unique . coerced) (collectTypeUniques ty <> t)
        WrapF _ tn ty t ->
            Set.insert (tn ^. unique . coerced) (collectTypeUniques ty <> t)

...
import qualified Language.PlutusCore.Check.Uniques
                                               as Uniques
...
fileNormalizeType norm =
    fmap (either prettyErr id . printNormalizeType norm) . BSL.readFile
  where
...
checkFile =
    fmap
            ( either (pure . prettyText) id
            . fmap (fmap prettyPlcDefText . check)
            . parse
            )
        . BSL.readFile
...
parseScoped =
    through (Uniques.checkProgram (const True)) <=< rename <=< parseProgram
...
typecheckPipeline
    ...
    -> m (NormalizedType TyNameWithKind ())
typecheckPipeline cfg = typecheckProgram cfg <=< annotateProgram <=< through
    (unless (_typeConfigNormalize cfg) . checkProgram)
...
applyProgram (Program _ _ t1) (Program _ _ t2) =
    Program () (defaultVersion ()) (Apply () t1 t2)

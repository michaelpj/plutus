...
import           Wallet.UTXO.Runtime            ( OracleValue(..)
                                                , Signed(..)
                                                )
import qualified Wallet.UTXO.Runtime           as Runtime

import           Language.Plutus.Coordination.Contracts.Future
                                                ( Future(..)
                                                , FutureData(..)
                                                )
import qualified Language.Plutus.Coordination.Contracts.Future
                                               as F
...
init w = outp <$> walletAction w (F.initialise (PubKey 1) (PubKey 2) contract)
  where
...
adjustMargin
    ...
    -> FutureData
    -> UTXO.Value
    -> Trace MockWallet TxOutRef'
adjustMargin w refs fd vl = outp
    <$> walletAction w (F.adjustMargin refs contract fd vl)
  where
    outp = snd . head . filter (isPayToScriptOut . fst) . txOutRefs . head
...
initialiseFuture = checkTrace $ do
    ...
    traverse_
        ...
        [(w1, startingBalance - initMargin), (w2, startingBalance - initMargin)]
...
settle = checkTrace $ do
    ...
    traverse_ (uncurry assertOwnFundsEq)
              [(w1, startingBalance + delta), (w2, startingBalance - delta)]
...
contract = Future
    ...
    where im = penalty + (Runtime.Value units * forwardPrice `div` 20) -- 5%
...
marginRange = (forwardPrice - delta, forwardPrice + delta)
    where delta = forwardPrice `div` 20
...
checkTrace t = property $ do
    let ib = Map.fromList
            [(PubKey 1, startingBalance), (PubKey 2, startingBalance)]
...
updateAll = processPending >>= void . walletsNotifyBlock [w1, w2]

...
import           Language.Plutus.Coordination.Contracts.Vesting
                                                ( Vesting(..)
                                                , VestingData(..)
                                                , VestingTranche(..)
                                                , retrieveFunds
...
commit w vv vl = exScriptOut <$> walletAction w (void $ vestFunds vv vl)
  where
    exScriptOut =
        snd . head . filter (isPayToScriptOut . fst) . txOutRefs . head
...
canRetrieveFunds = checkVestingTrace scen1 $ do
    ...
    traverse_ (uncurry assertOwnFundsEq)
              [(w2, w2Funds), (w1, startingBalance + 150)]
...
canRetrieveFundsAtEnd = checkVestingTrace scen1 $ do
    ...
    -- vesting scheme.
    traverse_ (uncurry assertOwnFundsEq)
              [(w2, w2Funds), (w1, startingBalance + total)]

...
data Swap = Swap
    ...
    , swapOracle          :: !PubKey -- ^ Public key of the oracle (see note [Oracles] in [[Language.Plutus.Coordination.Lib]])
    }
...
swapValidator _ = PlutusTx result where
    result = plc (\(redeemer :: OracleValue (Ratio Int)) () PendingTx{..} Swap{..} ->
        let
            ...
            extractVerifyAt :: OracleValue (Ratio Int) -> PubKey -> Int -> BlockHeight -> t
            ...
            -- participants.
            Tx (t1, t2) (o1, o2) = pendingTxTransaction

            -- Each participant must deposit the margin

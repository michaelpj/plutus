...
import qualified Language.PlutusCore.Constant.Dynamic
                                               as PLC
...
import qualified Language.PlutusIR.Compiler.Definitions
                                               as PIR
import qualified Language.PlutusIR.Optimizer.DeadCode
                                               as PIR
...
install args todo =
    ...
    in  pure (GHC.CoreDoPluginPass "Core to PLC" (pluginPass opts) : todo)
...
pluginPass opts guts = qqMarkerName >>= \case
    ...
    Nothing -> pure guts
    Just name ->
        GHC.bindsOnlyPass (mapM $ convertMarkedExprsBind opts name) guts
...
failCompilationSDoc message sdoc =
    liftIO $ GHC.throwGhcExceptionIO $ GHC.PprProgramError
...
thNameToGhcNameOrFail name = do
    ...
    case maybeName of
        ...
        Nothing ->
            failCompilation
                $  "Unable to get Core name needed for the plugin to function: "
                ++ show name
...
makeByteStringLiteral bs = do
    ...
    let
        upioed = GHC.mkCoreApps (GHC.Var upio)
                                [GHC.Type (GHC.mkTyConTy bsTc), upaled]
    ...
    = let
      ...
      in
        ...
        -- the ignored argument is the type for the polymorphic 'plc'
              e@(GHC.App (GHC.App (GHC.App (GHC.Var fid) (GHC.Type (GHC.isStrLitTy -> Just fs_locStr))) (GHC.Type _)) inner)
                  | markerName == GHC.idName fid
                  -> let vtype  = GHC.varType fid
                         locStr = show fs_locStr
                     in  case qqMarkerType vtype of
                             Just t  -> convertExpr opts locStr inner t
                             Nothing -> failCompilationSDoc
                                 "Found invalid marker, could not decode type"
                                 (GHC.ppr e)
              e@(GHC.Var fid) | markerName == GHC.idName fid ->
                  failCompilationSDoc
                      "Found invalid marker, not applied correctly"
                      (GHC.ppr e)
...
stringBuiltins =
    PLC.insertDynamicBuiltinNameDefinition PLC.dynamicCharToStringDefinition
        $ PLC.insertDynamicBuiltinNameDefinition PLC.dynamicAppendDefinition
                                                 mempty
...
stringBuiltinTypes =
    let normalStringBuiltinTypes = PLC.unDynamicBuiltinNameTypes
            $ PLC.dynamicBuiltinNameMeaningsToTypes stringBuiltins
        traceType = PLC.typeSchemeToType
            $ PLC.dynamicCallTypeScheme (PLC.TypedBuiltinDyn @String)
        allStringBuiltinTypes =
            Map.insert PLC.dynamicTraceName traceType normalStringBuiltinTypes
...
convertExpr opts locStr origE resType = do
    ...
    let result =
            withContextM
                    (sdToTxt $ "Converting expr at" GHC.<+> GHC.text locStr)
                $ do
                      (pirP :: PIRProgram) <-
                          PIR.Program ()
                          .   PIR.removeDeadBindings
                          <$> (PIR.runDefT () $ convExprWithDefs origE)
                      (plcP :: PLCProgram) <-
                          void
                              <$> ( flip runReaderT PIR.NoProvenance
                                  $ PIR.compileProgram pirP
                                  )
                      when (poDoTypecheck opts) $ do
                          annotated <- PLC.annotateProgram plcP
                          void $ PLC.typecheckProgram
                              (PLC.TypeConfig True
                                              stringBuiltinTypes
                                              (Just 1000)
                              )
        ...
        context = ConvertingContext
            { ccOpts            = ConversionOptions
                { coCheckValueRestriction = poDoTypecheck opts
                }
            , ccFlags           = flags
    ...
    case runExcept . runQuoteT . flip runReaderT context $ result of
        Left s ->
            let shown = show $ if poStripContext opts
                    then PP.pretty (stripContext s)
                    else PP.pretty s
            in  if poDeferErrors opts
           -- TODO: is this the right way to do either of these things?
                    then pure $ GHC.mkRuntimeErrorApp GHC.rUNTIME_ERROR_ID
                                                      resType
                                                      shown -- this will blow up at runtime

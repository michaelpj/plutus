...
module Language.PlutusTx.Compiler.Expr
    ( convExpr
    , convExprWithDefs
    , convDataConRef
    )
...
convLiteral = \case
    -- TODO: better sizes
    GHC.MachInt64 i ->
        pure $ PIR.Constant () $ PLC.BuiltinInt () haskellIntSize i
    GHC.MachInt i ->
        pure $ PIR.Constant () $ PLC.BuiltinInt () haskellIntSize i
    GHC.MachStr bs ->
        ...
        let str       = bsToStr (BSL.fromStrict bs)
    ...
    GHC.MachChar c -> do
        ...
        case maybeEncoded of
            ...
            Nothing ->
                throwPlain $ UnsupportedError "Conversion of character failed"
    GHC.LitInteger _ _ ->
        throwPlain $ UnsupportedError "Literal (unbounded) integer"
...
convExpr e = withContextM (sdToTxt $ "Converting expr:" GHC.<+> GHC.ppr e) $ do
    ...
    case e of
        ...
        GHC.App (GHC.App (GHC.Var n@(GHC.idDetails -> GHC.ClassOpId (GHC.getName -> className)))
            ...
            -> case className of
                ...
                ((==) GHC.ordClassName -> True) ->
                    convOrdMethod (GHC.getName n)
                ((==) GHC.numClassName -> True) ->
                    convNumMethod (GHC.getName n)
                ((==) GHC.integralClassName -> True) ->
                    convIntegralMethod (GHC.getName n)
                _ ->
        ...
        -- locally bound vars
        GHC.Var (lookupName top . GHC.getName -> Just (PIR.VarDecl _ name _))
            -> pure $ PIR.Var () name
        -- Special kinds of id
        ...
        GHC.Var (GHC.idDetails -> GHC.RecSelId{}      ) -> throwPlain
            $ UnsupportedError "Record selectors, use pattern matching"
        GHC.Var n -> do
        ...
        GHC.Let (GHC.Rec bs) body -> withVarsScoped (fmap fst bs) $ \vars -> do
        ...
        GHC.Case scrutinee b t alts -> do
            ...
            withVarScoped b $ \v -> do
                ...
                isValueAlt <- forM
                    alts
                    (\(_, vars, body) -> if null vars
                        then PIR.isTermValue <$> convExpr body
                        else pure True
                    )
                ...
                instantiated <-
                    PIR.TyInst () matched
                        <$> (convType t >>= maybeDelayType lazyCase)
                ...
                (tc, argTys) <- case GHC.splitTyConApp_maybe scrutineeType of
                    ...
                    Nothing ->
                        throwPlain
                            $ ConversionError
                                  "Scrutinee's type was not a type constructor application"
                dcs      <- getDataCons tc

                branches <- forM dcs $ \dc ->
                    case GHC.findAlt (GHC.DataAlt dc) alts of
                        Just alt ->
                            let
                                ...
                                instArgTys =
                                    GHC.dataConInstOrigArgTys dc argTys
                            in  convAlt lazyCase instArgTys alt
                        Nothing ->
                            throwPlain $ ConversionError
                                "No case matched and no default case"
        ...
        GHC.Cast body coerce -> do
            ...
            case splitNewtypeCoercion coerce of
                Just (Unwrap outer inner) -> do
                    ...
                    let instantiated =
                            PIR.TyInst () (PIR.Apply () match body') inner'
                ...
                _ ->
                    throwSd UnsupportedError $ "Coercion" GHC.$+$ GHC.ppr coerce
        GHC.Type _ ->
            throwPlain
                $ ConversionError
                      "Cannot convert types directly, only as arguments to applications"
        GHC.Coercion _ ->
            throwPlain $ ConversionError "Coercions should not be converted"

...
import qualified Language.PlutusIR.Compiler.Definitions
                                               as PIR
...
convType t = withContextM (sdToTxt $ "Converting type:" GHC.<+> GHC.ppr t) $ do
    ...
    case t of
        -- in scope type name
        (GHC.getTyVar_maybe -> Just (lookupTyName top . GHC.getName -> Just (PIR.TyVarDecl _ name _)))
            -> pure $ PIR.TyVar () name
        (GHC.getTyVar_maybe -> Just v) ->
            throwSd FreeVariableError $ "Type variable:" GHC.<+> GHC.ppr v
        (GHC.splitFunTy_maybe -> Just (i, o)) ->
            PIR.TyFun () <$> convType i <*> convType o
        (GHC.splitTyConApp_maybe -> Just (tc, ts)) -> convTyConApp tc ts
        (GHC.splitForAllTy_maybe -> Just (tv, tpe)) ->
            mkTyForallScoped tv (convType tpe)
...
convTyConApp tc ts
    ...
    -- we don't support Integer
      GHC.getName tc == GHC.integerTyConName = throwPlain
    $ UnsupportedError "Integer: use Int instead"
    |
...
convTyCon tc = do
    ...
    case maybeDef of
        ...
        Nothing -> do
            ...
            matchName <-
                safeFreshName ()
                $  (GHC.getOccString $ GHC.getName tc)
                ++ "_match"
            ...
            withTyVarsScoped (GHC.tyConTyVars tc) $ \tvs -> do
                ...
                PIR.defineDatatype tcName
                                   (PIR.Def tvd datatype)
                                   (Set.fromList deps)
...
splitNewtypeCoercion coerce = case GHC.coercionKind coerce of
    (GHC.Pair lhs@(GHC.splitTyConApp_maybe -> Just (GHC.unwrapNewTyCon_maybe -> Just (_, inner, _), _)) rhs)
        | GHC.eqType rhs inner
        -> Just $ Unwrap lhs rhs
    (GHC.Pair lhs rhs@(GHC.splitTyConApp_maybe -> Just (GHC.unwrapNewTyCon_maybe -> Just (_, inner, _), _)))
        | GHC.eqType lhs inner
        -> Just $ Wrap lhs rhs
    _ -> Nothing
...
sortConstructors tc cs =
    ...
    let sorted = sortBy
            (\dc1 dc2 -> compare (GHC.getOccName dc1) (GHC.getOccName dc2))
            cs
    in  if tc == GHC.boolTyCon || tc == GHC.listTyCon
            then reverse sorted
            else sorted
  ...
  where
    extractDcs tc
        | GHC.isAlgTyCon tc || GHC.isTupleTyCon tc = case GHC.algTyConRhs tc of
            GHC.AbstractTyCon ->
                throwSd UnsupportedError $ "Abstract type:" GHC.<+> GHC.ppr tc
...
mkConstructorType dc
    = let argTys = GHC.dataConRepArgTys dc
      in
          ...
          withContextM
                  (       sdToTxt
                  $       "Converting data constructor type:"
                  GHC.<+> GHC.ppr dc
                  )
              $ do
...
getConstructors tc = do
    ...
    case maybeConstrs of
        ...
        Nothing ->
            throwPlain $ ConversionError "Constructors have not been converted"
...
getConstructorsInstantiated t =
    withContextM
            (       sdToTxt
            $       "Creating instantiated constructors for type:"
            GHC.<+> GHC.ppr t
            )
        $ case t of
              ...
              -- must be a TC app
              _ ->
                  throwPlain $ ConversionError
                      "Type was not a type constructor application"
...
getMatchInstantiated t =
    withContextM
            (       sdToTxt
            $       "Creating instantiated matcher for type:"
            GHC.<+> GHC.ppr t
            )
        $ case t of
              (GHC.splitTyConApp_maybe -> Just (tc, args)) -> do
...
convAlt
    ...
    -> m PIRTerm
convAlt mustDelay instArgTys (alt, vars, body) =
    withContextM (sdToTxt $ "Creating alternative:" GHC.<+> GHC.ppr alt)
        $ case alt of
              ...
              GHC.DEFAULT  -> do
                  ...
                  argNames <- forM
                      [0 .. (length argTypes - 1)]
                      (\i -> safeFreshName () $ "default_arg" ++ show i)
                  pure $ PIR.mkIterLamAbs
                      ()
                      (zipWith (PIR.VarDecl ()) argNames argTypes)
                      body'
              ...
              GHC.DataAlt _ -> mkIterLamAbsScoped
                  vars
                  (convExpr body >>= maybeDelay mustDelay)

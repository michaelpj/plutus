...
convOrdMethod name
    ...
    -- only this one has a name defined in the lib??
      name == GHC.geName
    = lookupBuiltinTerm 'Builtins.greaterThanEqInteger
    | GHC.getOccString name == ">"
    = lookupBuiltinTerm 'Builtins.greaterThanInteger
    | GHC.getOccString name == "<="
    = lookupBuiltinTerm 'Builtins.lessThanEqInteger
    | GHC.getOccString name == "<"
    = lookupBuiltinTerm 'Builtins.lessThanInteger
    | otherwise
    = throwSd UnsupportedError $ "Ord method:" GHC.<+> GHC.ppr name
...
convIntegralMethod name
    | GHC.getOccString name == "div"
    = lookupBuiltinTerm 'Builtins.divideInteger
    | GHC.getOccString name == "rem"
    = lookupBuiltinTerm 'Builtins.remainderInteger
    | otherwise
    = throwSd UnsupportedError $ "Integral method:" GHC.<+> GHC.ppr name

...
tests = testNested
    "Plugin"
    [basic, primitives, structure, datat, recursiveTypes, recursion, errors]

basic :: TestNested
basic = testNested "basic" [goldenPlc "monoId" monoId, goldenPlc "monoK" monoK]
...
primitives = testNested
  ...
  --, goldenPlc "blocknum" blocknumPlc
    ...
    , goldenEval
        "bytestringApply"
        [ getAst bytestring
        , trivialProgram $ runQuote $ unsafeLiftPlc ("hello" :: ByteString)
        ]
...
ifThenElse :: PlcCode
ifThenElse =
    plc @"ifThenElse" (\(x :: Int) (y :: Int) -> if x == y then x else y)
...
verify :: PlcCode
verify = plc @"verify"
    (\(x :: ByteString) (y :: ByteString) (z :: ByteString) ->
        Builtins.verifySignature x y z
    )

structure :: TestNested
structure = testNested "structure" [goldenPlc "letFun" letFun]
...
datat :: TestNested
datat = testNested "data" [monoData, polyData, newtypes]
...
monoCase :: PlcCode
monoCase = plc @"monoCase"
    (\(x :: MyMonoData) -> case x of
        Mono1 _ b -> b
        Mono2 a   -> a
        Mono3 a   -> a
    )
...
irrefutableMatch = plc @"irrefutableMatch"
    (\(x :: MyMonoData) -> case x of
        Mono2 a -> a
    )

atPattern :: PlcCode
atPattern = plc @"atPattern"
    (\t@(x :: Int, y :: Int) -> let fst (a, b) = a in y + fst t)
...
nonValueCase = plc @"nonValueCase"
    (\(x :: MyEnum) -> case x of
        Enum1 -> 1 :: Int
        Enum2 -> Builtins.error ()
    )
...
defaultCasePoly = plc @"defaultCasePoly"
    (\(x :: MyPolyData Int Int) -> case x of
        Poly1 a _ -> a
        _         -> 2
    )
...
nestedNewtypeMatch =
    plc @"nestedNewtypeMatch" (\(MyNewtype2 (MyNewtype x)) -> x)
...
valueRestriction = plc @"valueRestriction"
    (let f :: forall a . a
         f = Builtins.error ()
     in  (f @Bool, f @Int)
    )

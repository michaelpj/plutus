...
import qualified Language.PlutusIR.Analysis.Dependencies
                                               as Deps
...
removeDeadBindings t =
    let
        ...
        liveNodes :: Liveness
        liveNodes =
            Set.fromList $ AM.reachable Deps.Root (T.toAdjacencyMap depGraph)
    in
        runReader (processTerm t) liveNodes

type Liveness = Set.Set Deps.Node

live :: (MonadReader Liveness m, PLC.HasUnique n unique) => n -> m Bool
live n =
    let u = coerce $ n ^. PLC.unique in asks $ Set.member (Deps.Variable u)

liveBinding
    :: ( MonadReader Liveness m
       , PLC.HasUnique (name a) PLC.TermUnique
       , PLC.HasUnique (tyname a) PLC.TypeUnique
       )
    ...
    = let
      ...
      in
          case b of
              ...
              DatatypeBind _ (Datatype _ d _ destr constrs) ->
                  or
                      <$> (  sequence
                          $  [liveTyVarDecl d, live destr]
                          ++ fmap liveVarDecl constrs
                          )

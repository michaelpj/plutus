...
runDefT
    :: (Monad m, Ord key)
    => ann
    -> DefT key ann m (Term TyName Name ann)
...
runDefT x act = do
  ...
  where
    bindingDefs defs
        = let
              terms = mapDefs
                  (\d -> TermBind x (PLC.defVar d) (PLC.defVal d))
                  (_termDefs defs)
              types = mapDefs
                  (\d -> TypeBind x (PLC.defVar d) (PLC.defVal d))
                  (_typeDefs defs)
              datatypes = mapDefs (\d -> DatatypeBind x (PLC.defVal d))
                                  (_datatypeDefs defs)
          in
...
defSccs tds
    = let
          inputs = fmap (\(key, (d, deps)) -> (d, key, Set.toList deps))
                        (Map.assocs tds)
      in  Graph.stronglyConnComp inputs
...
defineTerm
    :: MonadDefs key ann m
    => key
    -> TermDef TyName Name ann
    -> Set.Set key
    -> m ()
defineTerm name def deps =
    liftDef $ modify $ over termDefs $ Map.insert name (def, deps)

defineType
    :: MonadDefs key ann m => key -> TypeDef TyName ann -> Set.Set key -> m ()
defineType name def deps =
    liftDef $ modify $ over typeDefs $ Map.insert name (def, deps)

defineDatatype
    :: forall key ann m
     . MonadDefs key ann m
    => key
    -> DatatypeDef TyName Name ann
    -> Set.Set key
    -> m ()
defineDatatype name def deps =
    liftDef $ modify $ over datatypeDefs $ Map.insert name (def, deps)
...
lookupType x name = do
    DefState { _typeDefs = tys, _datatypeDefs = dtys, _aliases = as } <- liftDef
        get
    pure $ case Map.lookup name tys of
        Just (def, _) -> Just $ if Set.member name as
            then PLC.defVal def
            else mkTyVar x $ PLC.defVar def
        Nothing -> case Map.lookup name dtys of
            Just (def, _) -> Just $ mkTyVar x $ PLC.defVar def
            Nothing       -> Nothing

lookupTerm
    :: (MonadDefs key ann m) => ann -> key -> m (Maybe (Term TyName Name ann))
lookupTerm x name = do
    DefState { _termDefs = ds, _aliases = as } <- liftDef get
    pure $ case Map.lookup name ds of
        Just (def, _) -> Just $ if Set.member name as
            then PLC.defVal def
            else mkVar x $ PLC.defVar def
        Nothing -> Nothing
...
lookupConstructors x name = do
    ...
    pure $ case Map.lookup name ds of
        Just (PLC.Def { PLC.defVal = (Datatype _ _ _ _ constrs) }, _) ->
            Just $ fmap (mkVar x) constrs
        Nothing -> Nothing

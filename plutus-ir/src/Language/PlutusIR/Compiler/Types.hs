...
import qualified Language.PlutusCore.StdLib.Type
                                               as Types
...
mkRecursiveType
    :: Provenance a
    -> PLC.TyName (Provenance a)
    -> PLCType a
    -> Quote (Types.RecursiveType PLC.TyName (Provenance a))
mkRecursiveType ann tn pf = pure
    $ Types.RecursiveType (\t -> PLC.Wrap ann tn pf t) (PLC.TyFix ann tn pf)
...
wrap p r t = case r of
    PlainType _ -> t
    RecursiveType Types.RecursiveType { Types._recursiveWrap = wrapper } ->
        setProvenance p $ wrapper t

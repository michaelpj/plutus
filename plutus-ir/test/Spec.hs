...
import qualified Language.PlutusCore.StdLib.Data.Bool
                                               as Bool
import qualified Language.PlutusCore.StdLib.Data.Nat
                                               as Nat
import qualified Language.PlutusCore.StdLib.Data.Unit
                                               as Unit
import qualified Language.PlutusCore.StdLib.Meta
                                               as Meta
...
compileAndMaybeTypecheck
    ...
    -> Except (Error (Provenance a)) (PLC.Term TyName Name (Provenance a))
compileAndMaybeTypecheck doTypecheck pir =
    flip runReaderT NoProvenance
        $ runQuoteT
        $ do
    ...
    -- names during compilation that are not fresh
              ...
              when doTypecheck
                  ...
                  $ do
                        ...
                        PLC.typecheckTerm
                            (PLC.TypeConfig True mempty Nothing)
                            annotated
              pure compiled

tests :: TestNested
tests = testGroup "plutus-ir"
    <$> sequence [prettyprinting, datatypes, recursion, errors, optimizer]

prettyprinting :: TestNested
prettyprinting = testNested
    "prettyprinting"
    [goldenPir "basic" basic, goldenPir "maybe" (runQuote maybePir)]

basic :: Term TyName Name ()
basic = runQuote $ do
    a <- freshTyName () "a"
    x <- freshName () "x"
    pure $ TyAbs () a (Type ()) $ LamAbs () x (TyVar () a) $ Var () x
...
maybePir = do
    ...
    pure $ Let () NonRec [DatatypeBind () mb] $ Apply
        ()
        (TyInst () (mkVar () just) unit)
        unitval
...
listMatch = do
    ...
    pure
        ...
        $ mkIterApp
              ()
              (TyInst () (Apply () unitMatch unitNil) unit)
              [
                    -- nil case
                unitval
              ,
                ...
                mkIterLamAbs
                      ()
                      [ VarDecl () h unit
                      , VarDecl () t (TyApp () (mkTyVar () l) unit)
                      ]
                  $ Var () h
              ]
...
datatypes = testNested
    ...
    , goldenPlc
        ...
        $   trivialProgram
        .   void
        <$> compileAndMaybeTypecheck False listMatch
        )
    , goldenEval
        "listMatchEval"
        [ asIfThrown
...
recursion = testNested
    "recursion"
    [goldenPlc "even3" evenOdd, goldenEval "even3Eval" [evenOdd]]
...
evenOdd = do
    ...
    let
        eoFunc b recc = do
            ...
            pure
                $ LamAbs () n nat
                $ Apply () (Apply () (TyInst () (Unwrap () (Var () n)) bool) b)
                $ Var () recc
    ...
    pure
        $ Let () NonRec [TermBind () (VarDecl () arg nat) three]
        $ Let
              ()
              Rec
...
mutuallyRecursiveTypes = do
    ...
    pure
        $ Let () Rec [DatatypeBind () treeDt, DatatypeBind () forestDt]

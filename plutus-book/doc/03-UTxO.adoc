...
formal specification, see <<bibliography#bibliography>>.
...
Each element of the inputs set is a pair of a transaction ID and an index.
...
That is, if `tx` contains a pair of a list of inputs and a finite map
of inputs `(ins, outs)` with ID `id`, and

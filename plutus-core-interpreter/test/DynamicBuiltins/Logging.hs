...
module DynamicBuiltins.Logging (test_logging) where
...
dynamicIntToStringMeaning = DynamicBuiltinNameMeaning sch show
  where
...
evaluateHandlersCek
    ...
    -> r
evaluateHandlersCek =
    evaluateHandlersBy (\means -> Identity . evaluateCek means)
...
test_logInt = testCase "logInt" $ do
    ...
    let eval1 =
            evaluateHandlersCek (handleDynamicIntToString . handleDynamicLog)
    let eval2 =
            evaluateHandlersCek (handleDynamicLog . handleDynamicIntToString)
    ...
    let
        term = runQuote $ do
            ...
            return $ mkIterApp
                ...
                , mkIterApp
                    ()
                    (TyInst () biEnumFromTo (TyInt () size))
                    [ Constant () $ BuiltinInt () size 1
...
test_logging = testGroup "logging" [test_logInt, test_logInts]

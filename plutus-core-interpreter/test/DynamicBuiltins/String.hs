...
module DynamicBuiltins.String (test_dynamicStrings) where
...
test_stringRoundtrip = testProperty "stringRoundtrip" . property $ do
    ...
    let mayStr' =
            runQuote (makeDynamicBuiltin str)
                >>= sequence
...
test_plcListOfStringsRoundtrip =
    testProperty "listOfStringsRoundtrip" . property $ do
        strs <-
            forAll . fmap PlcList . Gen.list (Range.linear 0 10) $ Gen.string
                (Range.linear 0 10)
                Gen.unicode
        let mayStrs' =
                runQuote (makeDynamicBuiltin strs)
                    >>= sequence
                    .   readDynamicBuiltinCek
...
test_collectChars = testProperty "collectChars" . property $ do
    str              <- forAll $ Gen.string (Range.linear 0 20) Gen.unicode
    (str', errOrRes) <-
        liftIO . withEmitEvaluateBy evaluateCekCatch TypedBuiltinDyn $ \emit ->
            runQuote $ do
                unit    <- getBuiltinUnit
                ...
                ignore  <- do
                    x <- freshName () "x"
                    y <- freshName () "y"
                    return . LamAbs () x unit . LamAbs () y unit $ Var () y
                let step arg rest =
                        mkIterApp () ignore [Apply () emit arg, rest]
...
test_dynamicStrings = testGroup
    "dynamicStrings"
    [test_stringRoundtrip, test_plcListOfStringsRoundtrip, test_collectChars]

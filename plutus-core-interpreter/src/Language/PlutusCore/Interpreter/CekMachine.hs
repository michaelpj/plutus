...
extendVarEnv argName arg argVarEnv (VarEnv oldVarEnv) = VarEnv $ IntMap.insert
    (unUnique $ nameUnique argName)
    (Closure argVarEnv arg)
    oldVarEnv
...
lookupVarName varName = do
    ...
    case IntMap.lookup (unUnique $ nameUnique varName) varEnv of
        Nothing -> throwError $ MachineException
...
lookupDynamicBuiltinName dynName = do
    ...
    case Map.lookup dynName means of
        Nothing -> throwError $ MachineException err term
          where
...
computeCek con (Wrap ann tyn ty term) =
    computeCek (FrameWrap ann tyn ty : con) term
...
returnCek (FrameWrap ann tyn ty : con) val =
    returnCek con $ Wrap ann tyn ty val
...
instantiateEvaluate
    :: Context -> Type TyName () -> Plain Term -> CekM EvaluationResult
...
instantiateEvaluate con ty fun
    ...
    $ MachineException NonPrimitiveInstantiationMachineError fun
      ...
      in
          case termAsPrimIterApp term of
              ...
              Just (IterApp headName spine) -> do
                  constAppResult <- runQuoteT
                      $ applyStagedBuiltinName headName spine
                  withVarEnv funVarEnv $ case constAppResult of
                      ...
                      ConstAppError err   -> throwError
                          $ MachineException (ConstAppMachineError err) term

evaluateInCekM :: Evaluate (Either CekMachineException) a -> CekM a
evaluateInCekM a = ReaderT $ \cekEnv ->
    let eval means' =
            evaluateCekCatchIn $ cekEnv & cekEnvMeans %~ mappend means'
    in  runEvaluate eval a

-- | Apply a 'StagedBuiltinName' to a list of 'Value's.
applyStagedBuiltinName
    :: StagedBuiltinName -> [Plain Value] -> QuoteT CekM ConstAppResult
...
evaluateCekCatch
    ...
    -> Plain Term
    -> Either CekMachineException EvaluationResult
evaluateCekCatch means =
    evaluateCekCatchIn (CekEnv means $ VarEnv IntMap.empty)

-- | Evaluate a term using the CEK machine. May throw a 'CekMachineException'.
evaluateCek
    :: DynamicBuiltinNameMeanings -> Term TyName Name () -> EvaluationResult
evaluateCek = either throw id .* evaluateCekCatch

readDynamicBuiltinCek
    :: KnownDynamicBuiltinType dyn
    => Term TyName Name ()
    -> Either CekMachineException (Maybe dyn)
...
runCek
    :: DynamicBuiltinNameMeanings -> Program TyName Name () -> EvaluationResult

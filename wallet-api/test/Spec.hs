...
tests = testGroup
  ...
  , testGroup "Etc." [testProperty "splitVal" splitVal]
  ]
...
txnIndexValid = property $ do
  (m, txn) <- forAll genChainTxn
  let
    (result, st) = Gen.runTrace m processPending
    idx          = _index st
  Hedgehog.assert
    (Right () == Index.runValidation (Index.validateTransaction 0 txn) idx)
...
invalidScript = property $ do
  ...
  let
    scriptTxn = txn1 & outputs . element index %~ \o ->
      scriptTxOut (txOutValue o) failValidator unitData
 ...
 where
  ...
  failValidator =
    Validator
      $ fromPlcCode
          $$(plutus [|| \() () () -> $$(traceH) "I always fail everything" (Builtins.error @()) ||])
...
notifyWallet = property $ do
  let w = Wallet 1
  (e, EmulatorState { _walletStates = st }) <-
    forAll
  ...
  Hedgehog.assert
    $  (getSum . foldMap Sum . view ownFunds <$> ttl)
    == Just initialBalance
...
eventTrace = property $ do
  ...
  (e, EmulatorState { _walletStates = st }) <-
    ...
    $ do
        ...
        let
          mkPayment = EventHandler $ \_ -> void $ payToPubKey 100 (PubKey 2)
  ...
  Hedgehog.assert $ (getSum . foldMap Sum . view ownFunds <$> ttl) == Just
    (initialBalance - 100)

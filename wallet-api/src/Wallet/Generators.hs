...
generatorModel = GeneratorModel
    { gmInitialBalance = Map.fromList $ first PubKey <$> zip [1 .. 5]
                                                             (repeat 100000)
...
genInitialTransaction :: MonadGen m => GeneratorModel -> m (Tx, [TxOut'])
...
genValidTransaction'
    :: MonadGen m => GeneratorModel -> FeeEstimator -> Mockchain -> m Tx
...
genValidTransactionSpending :: MonadGen m => Set.Set TxIn' -> Value -> m Tx
genValidTransactionSpending =
    genValidTransactionSpending' generatorModel (constantFee 1)
...
genValidTransactionSpending' g f ins totalVal = do
    ...
    if fee < totalVal
        then do
            ...
            pure Tx
                { txInputs     = ins
                , txOutputs    = uncurry pubKeyTxOut
                    <$> zip outVals (Set.toList $ gmPubKeys g)
                , txForge      = 0
                , txFee        = fee
                , txSignatures = []
                }
...
assertValid tx (Mockchain ch _) =
    Hedgehog.assert (validTx unitValidationData tx [ch])
...
splitVal mx init' = go 0 0 []
  where
    go i c l = if i >= pred mx
        then pure $ (init' - c) : l
        else do
            v <- Gen.integral (Range.linear 1 $ init' - c)
            if v + c == init' then pure $ v : l else go (succ i) (v + c) (v : l)

...
import qualified Language.PlutusCore.Evaluation.CkMachine
                                               as PLC
import qualified Language.PlutusCore.Interpreter.CekMachine
                                               as PLC
import qualified Language.PlutusCore.Interpreter.LMachine
                                               as PLC
...
normalizationMode = option
    ...
    <> showDefault
    <> help
           "Whether type annotations must be normalized or not (one of Required or NotRequired)"
    )
...
runTypecheck (TypecheckOptions inp mode) = do
    ...
    let cfg = PLC.TypeConfig
            (case mode of
                NotRequired -> True
                Required    -> False
            )
            mempty
            PLC.defaultTypecheckerGas
